...
public class Phase11 {
	 	...
	 	private static Logger logger = Logger.getLogger("Phase1");  
	
		private static int data_buffer;
		private static ByteBuffer row_buff, col_buff;
		private static PageTable[] pg_row, pg_col;
		...
		public static void main(String args[]) throws IOException{
			...
			while(size_accepted){
				...
				if((data_buffer/512)%2!=0){
					//If odd convert to even for storage
					int odd_val=data_buffer/512;
					odd_val=odd_val-1;
					data_buffer=odd_val*512;
					...
					System.out.println("The Memory buffer size has been reset to "+data_buffer);
					size_accepted=false;
				}else{
					size_accepted=false;
				}
			...
			//Divide the database memory buffer into two equal parts for row and column
			row_buff=ByteBuffer.allocateDirect(data_buffer/2);
			col_buff=ByteBuffer.allocateDirect(data_buffer/2);
			
			//Create Page Tables for row and column
			int num_pages=(data_buffer/2)/512;
			
			pg_row=new PageTable[num_pages-1];
			for(int i=0;i<pg_row.length;i++){
				pg_row[i]=new PageTable();
			}
			
			pg_col=new PageTable[num_pages];
			...
			int num_of_files;
			System.out.println("Enter the number of files to be converted into column store");
			num_of_files = sc.nextInt();
			
			//Create MetaData Holders
			metaData=new MetaData[num_of_files];
			...
			{	
				...
				metaData[i].info="".concat(file).concat(";id clientname phone;4 16 12");
				//Get the table names from the user
				System.out.println("Enter the file to be converted to column store");
				file = sc.next();
				...
				//If file are found store them as column store (In disk according to the assignment)
				if(flag_file_exists){
					convert_to_columnstore(file,brFile);
					System.out.println("Converted to column store");
				}
			}
			
			String input;
			System.out.println("Enter s for giving script files names or q to quit: ");
			input = sc.next();
			while( !(input.equals("q") || input.equals("Q")) )
			{							
				String script;
				System.out.println("Enter the script file name: ");
				script = sc.next();
				script = script.concat(".txt");
				...
				while(query!=null){
					...
					if(type.contentEquals("R")||type.contentEquals("I")||type.contentEquals("D")){
						...
						if(!flag_file_exists && type.contentEquals("R")){
							//Aborted Log Error
							logger.info("Query Aborted: Table X does no Exist.\n");
							//Next Query
							query=br.readLine();
							continue;
				...
				br.close();
				System.out.println("Enter s for giving script files name or q to quit:");
				input = sc.next();				
			}
			sc.close();
		}
		...
		private static void read_phone(String table, String value) throws IOException{
			...
			while(pages_to_read!=pages_read)
			{
				...
				if(empty)
				{
					...
					pg_col[page_to_swap].hash_value = 10; //gave some number other than -1
				}
		...
		private static void retrieve_records(String table, String value) throws IOException{
			...
			{	
				...
				do
				{
					...
					if(empty)
					{
						...
						pg_col[page_to_swap].time=System.currentTimeMillis();
						pg_col[page_to_swap].hash_value = 0;
					}
					else//swap existing page
					{
						page_to_swap = 0;
			...
			else
			{
				...
				{
					logger.info((String) id_al.get(i)+" , "+name_al.get(i)+" , "+(String) phone_al.get(i));
				}
			}
		...
		private static void read(String table, String value) throws IOException{
				...
				for(int i=0;i<match_positions.length;i++){
					...
					if(match_positions[i]==1){
						...
						if(read_success){
							...
							String p1=String.valueOf(row_buff.getInt((i*512)+(index*32)+20));
							String p2=String.valueOf(row_buff.getInt((i*512)+(index*32)+24));
							String p3=String.valueOf(row_buff.getInt((i*512)+(index*32)+28));
			...
			if(!read_success){
				...
				}while(newPage.getInt(504)==504 && newPage.getInt(508)!=-1);
			}
			...
			if(empty && !storeflag){
				for(int i=0;i<empty_positions.length;i++){
					if(empty_positions[i]==1){
						...
						ByteBuffer newPage=getLast15Records(hash_value,table,i);
		...
		private static ByteBuffer getLast15Records(int hash_value, String table, int lru) throws IOException{
			...
			if(records_onLastPage==0){
				...
				if((records_onLastPage*4)<=end_value){
					...
					if(end_value==504){
						...
						//Set end and overflow pointers for the page to return
						read.putInt(504, records_onLastPage*4);
						read.putInt(508,new_bucket); 
		...
		private static void convert_to_columnstore(String fileName,BufferedReader reader) throws IOException{
